	...
	private void receiveMessage(Message msg) {
		if (msg.obj instanceof GameAction) { // ignore if not GameAction
			...
			if (action instanceof MyNameIsAction &&
				...
				Log.i("HeartsLocalGame", "received 'myNameIs' ("+mnis.getName()+")");
				...
				if (playerNameCount >= playerNames.length) {
					Log.i("HeartsLocalGame", "broadcasting player names");
				...
				Log.i("HeartsLocalGame", "got 'ready' ("+playerNames[playerIdx]+")");
				...
				if (playerReadyCount >= playerNames.length) {
					...
					Log.i("HeartsLocalGame", "broadcasting initial state");
